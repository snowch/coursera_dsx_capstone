...
```{r}
inspect(tdm[grep('aadil', terms), 1:10])
```
...
num_non_english_words <- as.integer(totalWords) - as.integer(num_english_words)
print( num_non_english_words )
```

Percentage of non-english words - this seems too high

```{r pct_non_english_words}
print( num_non_english_words / totalWords )
```
